...
I_zz = 0.67; % Moment of inertia around z-axis (kg-m^2)
...
m = 6.71; % Mass of the quadcopter (kg) % Old Mass = 6.71kg
...
Drag_on = 1; % Enable Drag force in model
Alt = 600; % Starting height of the Sea level (m)
h = Alt+10; % Starting height of the drone from Sea level (m)
...
TimeVector = seconds(0:numSamples(1,1)-1)' / fs;
% Convert to timetable
TS = array2timetable(input, 'RowTimes', TimeVector);

%{ These values are self set to limit region of operation in dynamics, 
% range can be extended based on test data available %}
RPM_max = 17668; % Maximum RPM of motor (rev/min), currently scales input 
